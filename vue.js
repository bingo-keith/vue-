...
function updateComponentListeners (
  ...
  function eventsMixin (Vue) {
    ...
    // $on方法监听当前实力上的自定义事件。事件可以由vm.$emit触发。回调函数会接收所有传入的事件触发函数的额外参数
  ...
  function initLifecycle (vm) {
    ...
    vm.$parent = parent; // 指定已创建的实例之父实例，在两者之间建立父子关系
  ...
  function queueWatcher (watcher) {
    ...
    if (has[id] == null) {
      ...
      if (!waiting) {
        ...
        nextTick(flushSchedulerQueue); //更新观察者 运行观察者watcher.run() 函数 并且   调用组件更新和激活的钩子
      }
    }
  }
