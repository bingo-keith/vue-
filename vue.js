...
(function (global, factory) {
  ...
  function initEvents (vm) {
    vm._events = Object.create(null); // vm._event 标识父组件绑定在当前组件上的事件
    vm._hasHookEvent = false; // 父组件是否通过@hook:把钩子函数绑定在当前组件上
    // init parent attached events
    var listeners = vm.$options._parentListeners; // 初始化父组件添加的事件
    if (listeners) {
  ...
  function eventsMixin (Vue) {
    ...
    Vue.prototype.$on = function (event, fn) {
      ...
      if (Array.isArray(event)) {
        ...
        // _events是标识直接绑定在组件上的事件，如果通过$on新添加的事件（也相当于直接绑定在组件上的事件），我们也要把事件和回调方法传入到_events对象中
